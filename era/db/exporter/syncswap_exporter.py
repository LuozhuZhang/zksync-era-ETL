      ...
      for row in reader:
        cur.execute(f"""
          INSERT INTO {full_table_name} (
            type, transaction_hash, block_number, block_time, from_address,
            to_address, from_token_address, from_token_amount,
            to_token_address, to_token_amount
...
def export_syncswap_swaps():
  ...
  csv_base_folder = os.path.join(current_file_path, '..', '..', '..', 'data', 'json_to_csv', 'era', 'all_syncswaps')
  parallel_import(db_config, csv_base_folder, 'era', 'syncswap_swaps', 'all_syncswaps', END_BLOCK, import_csv_to_db)
